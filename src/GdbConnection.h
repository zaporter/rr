...
public:
   ...
   */
  virtual void reply_select_thread(bool ok);
   ...
   */
  virtual void reply_get_reg(const GdbRegisterValue& value);

  /**
   * Send |file| back to the debugger host.  |file| may contain
   * undefined register values.
   */
  virtual void reply_get_regs(const std::vector<GdbRegisterValue>& file);
